...
import { useApp, Return, ItemCondition } from '@/context/AppContext';
...
export default function ReturnDetailPage() {
  ...
  const getCurrentStatusIndex = () => {
    ...
    return returnStatusFlow.indexOf(returnItem.status);
  };
  ...
  const handleStartInspection = () => {
    ...
    returnItem.items.forEach(item => {
      initial[item.productId] = { received: item.quantity, condition: 'like_new' };
    });
  ...
  const handleCompleteInspection = () => {
    ...
    updatedItems.forEach(item => {
      ...
      if (condition && ['new_sealed', 'new_opened', 'like_new', 'good'].includes(condition)) {
        dispatch({
          type: 'ADJUST_STOCK',
          payload: { productId: item.productId, locationId: 'loc-warehouse', adjustment: item.quantityReceived }
        });
      }
    });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <ReturnStatusBadge status={returnItem.status} size="md" />
              <ReturnChannelBadge channel={returnItem.channel} />
            </div>
            <p className="text-sm text-slate-400 mt-1">
              ...
              {' '}&bull; Created {formatReturnDateTime(returnItem.createdAt)}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          {returnItem.status === 'requested' && (
            <>
              <button onClick={handleApprove} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg transition-colors flex items-center gap-2">
                <i className="fas fa-check"></i>
                Approve Return
              </button>
              <button onClick={handleReject} className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white rounded-lg transition-colors flex items-center gap-2">
          ...
          {returnItem.status === 'approved' && (
            <button onClick={handleMarkInTransit} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg transition-colors flex items-center gap-2">
              <i className="fas fa-truck"></i>
              Mark In Transit
            </button>
          )}
          {returnItem.status === 'in_transit' && (
            <button onClick={handleMarkReceived} className="px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg transition-colors flex items-center gap-2">
              <i className="fas fa-box"></i>
              Mark Received
            </button>
          )}
          {returnItem.status === 'received' && !isInspecting && (
            <button onClick={handleStartInspection} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg transition-colors flex items-center gap-2">
              <i className="fas fa-search"></i>
              Start Inspection
            </button>
          )}
          {returnItem.status === 'inspected' && (
            <button onClick={handleProcessRefund} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg transition-colors flex items-center gap-2">
      ...
      {returnItem.status !== 'rejected' && (
        <div className="bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-xl p-6">
          ...
          <div className="flex items-center justify-between">
            {returnStatusFlow.map((status, index) => {
              ...
              return (
                <div key={status} className="flex flex-col items-center flex-1">
                  <div className="flex items-center w-full">
                    ...
                    {index < returnStatusFlow.length - 1 && (
          ...
          <div className="bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-xl p-6">
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right">
                      ...
                      {item.quantityReceived > 0 && !isInspecting && (
                        <p className="text-sm text-slate-400">{item.quantityReceived} of {item.quantity} received</p>
                      )}
                    </div>
                  </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-700/50 grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setInspectionData({
                            ...inspectionData,
                            [item.productId]: { ...inspectionData[item.productId], received: parseInt(e.target.value) || 0 }
                          })}
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => setInspectionData({
                            ...inspectionData,
                            [item.productId]: { ...inspectionData[item.productId], condition: e.target.value as ItemCondition }
                          })}
                          className="w-full bg-slate-700/50 border border-slate-600 rounded-lg px-3 py-2 text-white"
                        >
            ...
            {isInspecting && (
              <div className="mt-4 flex justify-end gap-3">
                <button onClick={() => setIsInspecting(false)} className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg transition-colors">
                  Cancel
                </button>
                <button onClick={handleCompleteInspection} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg transition-colors flex items-center gap-2">
                  <i className="fas fa-check"></i>
                  Complete Inspection
                </button>
              </div>
              ...
              {returnItem.refund.refundedAt && (
                <div className="pt-3 border-t border-slate-700">
                  <div className="flex items-center gap-2 text-emerald-400">
                    ...
                    <span className="text-sm">Refunded on {formatReturnDate(returnItem.refund.refundedAt)}</span>
                  </div>
            ...
            <div className="space-y-4">
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{formatReturnDateTime(returnItem.createdAt)}</p>
                </div>
              </div>
              {returnItem.updatedAt !== returnItem.createdAt && (
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{formatReturnDateTime(returnItem.updatedAt)}</p>
                  </div>
                </div>
              )}
            </div>
